...
local function loadInput(filename)
	-- TODO a better way of handling multi-sectioned input
	local instructions = ''
	local map = {}
	for line in io.lines(filename) do
		if #line > 0 then
			if #instructions == 0 and line:match'[RL]+' ~= nil then
				instructions = line
			else
				-- %w (alphanumerics), for part 2 test
				-- otherwise %u (upper case letters) will do
...
local function partOne(filename, expected)
	...
	local ins, mp = loadInput(filename)
